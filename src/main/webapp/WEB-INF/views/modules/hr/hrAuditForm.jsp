...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/hr/audit/form?id=${hrAudit.id}">
		<shiro:hasPermission name="oa:testAudit:edit">审批${not empty testAudit.id?'修改':'申请'}流程</shiro:hasPermission>
		<shiro:lacksPermission name="oa:testAudit:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="hrAudit" action="${ctx}/hr/audit/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="hr.taskId"/>
		<form:hidden path="hr.taskName"/>
		<form:hidden path="hr.taskDefKey"/>
		<form:hidden path="hr.procInsId"/>
		<form:hidden path="hr.procDefId"/>
		<form:hidden id="flag" path="hr.flag"/>
			...
			<table class="table-form">
				<tr>
					<td class="tit">姓名</td><td>
						...
						<sys:treeselect id="office" name="office.id" value="${hrAudit.office.id}" labelName="office.name" labelValue="${hrAudit.office.name}" 
		...
		<div class="form-actions">
				...
				<c:if test="${not empty hrAudit.id}">
		...
		<c:if test="${not empty hrAudit.id}">
			<act:histoicFlow procInsId="${hrAudit.hr.procInsId}" />
		</c:if>
	</form:form>
</body>
</html>
